...
static gboolean lg_graph_data_series_remove_all (PLGRAPH plg)
{
    ...
    {
        psd = data_sets->data;
        g_clear_pointer (&psd->lg_point_dvalue, g_free);
        g_clear_pointer (&psd->point_pos, g_free);
        g_clear_pointer (&psd, g_free);
        data_sets = g_list_next (data_sets);
        i_count++;
    }
...
static void lg_graph_set_x_label_text (PLGRAPH plg, gchar * pch_text)
{
    ...
    g_clear_pointer (&plg->x_label_text, g_free);
...
static void lg_graph_set_y_label_text (PLGRAPH plg, gchar * pch_text)
{
    ...
    g_clear_pointer (&plg->y_label_text, g_free);
...
static void lg_graph_set_chart_title (PLGRAPH plg, gchar * pch_text)
{
    ...
    g_clear_pointer (&plg->x_title_text, g_free);
...
static void cb_util_popup_menu_response_exit(GtkWidget * widget, gpointer gp)
{
   ...
   if (((PGAPC_MONITOR) gp)->cb_id == CB_MONITOR_ID) {
      ...
      pm = (PGAPC_MONITOR) gp;
      pm->b_run = FALSE;
      g_settings_set_boolean(pm->monitor_settings, GAPC_ENABLE_KEY, FALSE);
   } else {
...
static void cb_panel_property_color_reset (GtkButton *button, PGAPC_CONFIG pcfg)
{
   ...
   g_return_if_fail (pcfg->controller_settings != NULL);

   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_LINEV_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_LOADPCT_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_TIMELEFT_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_BCHARGE_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_BATTV_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_WINDOW_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_CHART_KEY);
   g_settings_reset(pcfg->controller_settings, GAPC_COLOR_TITLE_KEY);
...
static gboolean cb_util_line_chart_refresh_control(PGAPC_MONITOR pm)
{
   ...
   g_return_val_if_fail(pm != NULL, G_SOURCE_REMOVE);
...
static void sknet_net_shutdown (PSKCOMM psk)
{
    ...
    g_clear_pointer(&psk->gip, g_free);
...
static gint gapc_net_transaction_service(PGAPC_MONITOR pm, gchar * cp_cmd, gchar ** pch)
{
   ...
   for (iflag = 0; iflag < GAPC_MAX_ARRAY; iflag++) {
      g_clear_pointer(&pch[iflag], g_free);
   }
...
static int gapc_monitor_names_compare(
   const gchar * name1,
   const gchar * name2)
{
...
static int gapc_monitor_names_compare_data(
   const gchar * name1,
   const gchar * name2,
   gpointer user_data)
{
   return gapc_monitor_names_compare(name1, name2);
}

static void gapc_strv_builder_add_string(
   gchar *str,
...
static GQueue *gapc_monitor_names_queue_new(PGAPC_CONFIG pcfg)
{
   ...
   if (g_queue_get_length(monitor_names_queue) > 1) {
      g_queue_sort(
         monitor_names_queue,
         (GCompareDataFunc)gapc_monitor_names_compare_data,
         NULL);
   }
   return monitor_names_queue;
}

/*
 * Add a monitor to GSettings
 * returns FALSE on error
 * returns TRUE on success
*/
static gboolean gapc_panel_preferences_gsettings_add_monitor(PGAPC_CONFIG pcfg)
{
...
static void cb_panel_prefs_button_add_rec(GtkWidget * button, PGAPC_CONFIG pcfg)
{
   ...
   gapc_panel_preferences_gsettings_add_monitor(pcfg);
 ...
 *  We update GSettings here and GSettings updates the
 *  list_store in cb_panel_preference_gsettings_changed()
*/
   ...
   GAPC_PrefsType col_num)
{
   ...
   pcol->pht_Monitor_Settings = pcfg->pht_Monitor_Settings;
...
static void gapc_monitor_load(gchar *monitor_name, PGAPC_CONFIG pcfg)
{
   ...
   g_hash_table_insert(pcfg->pht_Monitor_Settings, g_strdup(monitor_name), monitor_settings);
...
/*
 * Gets the GSettings preferences for all monitors
...
static GtkWidget *gapc_panel_preferences_model_init(PGAPC_CONFIG pcfg)
{
   ...
   model = GTK_TREE_MODEL(gtk_list_store_new(GAPC_N_PREFS_COLUMNS,
         G_TYPE_UINT,              /* Monitor base-1 */
         G_TYPE_BOOLEAN,           /* enabled */
         G_TYPE_BOOLEAN,           /* systray icon */
         G_TYPE_UINT,              /* Port  */
         G_TYPE_DOUBLE,            /* network Refresh */
         G_TYPE_DOUBLE,            /* graph Refresh */
         G_TYPE_STRING,            /* Host  */
         G_TYPE_UINT               /* Wattage  */
      ));
   /* store it for later */
    ...
    */
   g_signal_connect_data(renderer_enabled, "toggled",
      G_CALLBACK(cb_panel_prefs_handle_cell_toggled),
      col_enabled, (GClosureNotify)g_free, G_CONNECT_AFTER);
   g_signal_connect_data(renderer_systray, "toggled",
      G_CALLBACK(cb_panel_prefs_handle_cell_toggled),
      col_systray, (GClosureNotify)g_free, G_CONNECT_AFTER);
   g_signal_connect_data(renderer_port, "edited",
      G_CALLBACK(cb_panel_prefs_handle_cell_edited),
      col_port, (GClosureNotify)g_free, G_CONNECT_AFTER);
   g_signal_connect_data(renderer_watt, "edited",
      G_CALLBACK(cb_panel_prefs_handle_cell_edited),
      col_watt, (GClosureNotify)g_free, G_CONNECT_AFTER);
   g_signal_connect_data(renderer_refresh, "edited",
      G_CALLBACK(cb_panel_prefs_handle_cell_edited),
      col_refresh, (GClosureNotify)g_free, G_CONNECT_AFTER);
   g_signal_connect_data(renderer_graph, "edited",
      G_CALLBACK(cb_panel_prefs_handle_cell_edited),
      col_graph, (GClosureNotify)g_free, G_CONNECT_AFTER);
   g_signal_connect_data(renderer_host, "edited",
      G_CALLBACK(cb_panel_prefs_handle_cell_edited),
      col_host, (GClosureNotify)g_free, G_CONNECT_AFTER);
...
static GtkWidget *gapc_panel_monitors_model_init(PGAPC_CONFIG pcfg)
{
   ...
   /* Create the model -- this column order and that of the enum must match */
   model = GTK_TREE_MODEL(gtk_list_store_new(GAPC_N_MON_COLUMNS, G_TYPE_UINT,   /* Monitor Num */
   ...
   gchar * pch_pversion, GdkPixbuf * icon)
{
   ...
   g_clear_pointer(&about_text, g_free);
   g_clear_pointer(&about_msg, g_free);
   ...
   PGAPC_CONFIG pcfg)
{
   ...
   gchar *monitor_settings_path = NULL;

   gboolean ov_b_tray = FALSE;
   guint ov_i_port = 0;
   guint ov_i_watt = 0;
   gdouble ov_f_graph = 0.0;
   gdouble ov_f_refresh = 0.0;
   ...
   if (b_ls_valid) {
      ...
      if (g_str_equal(key, GAPC_ENABLE_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled, -1);
         b_value = g_settings_get_boolean(monitor_settings, key);
      ...
      else if (g_str_equal(key, GAPC_SYSTRAY_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled,
            GAPC_PREFS_SYSTRAY, &ov_b_tray, -1);
         b_value = g_settings_get_boolean(monitor_settings, key);
      ...
      else if (g_str_equal(key, GAPC_PORT_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled,
      ...
      else if (g_str_equal(key, GAPC_WATT_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled,
      ...
      else if (g_str_equal(key, GAPC_REFRESH_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled,
            GAPC_PREFS_REFRESH, &ov_f_refresh, -1);
         d_value = g_settings_get_double(monitor_settings, key);
         if (d_value != ov_f_refresh) {
            gtk_list_store_set(GTK_LIST_STORE(pcfg->prefs_model), &iter,
               GAPC_PREFS_REFRESH, d_value, -1);
         }
         if ((b_m_enabled) && (b_active_valid)) {
            pm->d_refresh = d_value;
            pm->b_timer_control = TRUE;
         }
      }
      else if (g_str_equal(key, GAPC_GRAPH_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled,
            GAPC_PREFS_GRAPH, &ov_f_graph, -1);
         d_value = g_settings_get_double(monitor_settings, key);
         if (d_value != ov_f_graph) {
            gtk_list_store_set(GTK_LIST_STORE(pcfg->prefs_model), &iter,
               GAPC_PREFS_GRAPH, d_value, -1);
         }
         if ((b_m_enabled) && (b_active_valid)) {
            pm->d_graph = d_value;
      ...
      else if (g_str_equal(key, GAPC_HOST_KEY)) {
         gtk_tree_model_get(
            pcfg->prefs_model,
            &iter,
            GAPC_PREFS_ENABLED, &b_m_enabled,
         ...
         g_clear_pointer(&s_value, g_free);
...
static void gapc_signal_handler_monitor_settings_disconnect(
   ...
   PGAPC_CONFIG pcfg)
{
   guint number_of_handlers;

   g_return_if_fail(monitor_settings != NULL);

   number_of_handlers = g_signal_handlers_disconnect_by_func(
      monitor_settings,
      G_CALLBACK(cb_panel_preferences_gsettings_changed),
      pcfg);
}
...
static gboolean gapc_panel_gsettings_destroy(PGAPC_CONFIG pcfg)
{
   ...
   g_hash_table_foreach(
      pcfg->pht_Monitor_Settings,
      (GHFunc)gapc_signal_handler_monitor_settings_disconnect,
      pcfg);

   if (pcfg->pht_Monitor_Settings != NULL) {
      g_hash_table_destroy(pcfg->pht_Monitor_Settings);
      pcfg->pht_Monitor_Settings = NULL;
   }
   number_of_handlers = g_signal_handlers_disconnect_by_func(
...
static gboolean gapc_gsettings_watch(PGAPC_CONFIG pcfg)
{
   ...
   g_settings_bind(
      pcfg->controller_settings,
      GAPC_SYSTRAY_KEY,
      g_hash_table_lookup(pcfg->pht_Widgets, GAPC_SYSTRAY_KEY),
      "active",
...
/*
 * Gets the GSettings preferences for this program.
 * returns FALSE on error
 * returns TRUE on success
*/
...
static void cb_monitor_interface_destroy(GtkWidget * widget, PGAPC_MONITOR pm)
{
   ...
   for (h_index = 0; h_index < GAPC_MAX_ARRAY; h_index++) {
      g_clear_pointer(&pm->pach_events[h_index], g_free);
      g_clear_pointer(&pm->pach_status[h_index], g_free);
   }
...
static gint gapc_monitor_history_page(PGAPC_MONITOR pm, GtkWidget * notebook)
{
   ...
   pphs = (PGAPC_HISTORY) & pm->phs;
   pcfg = (PGAPC_CONFIG) pm->gp;
   ...
   pch_colors[0] = g_settings_get_string(pcfg->controller_settings, GAPC_COLOR_LINEV_KEY);
   pch_colors[1] = g_settings_get_string(pcfg->controller_settings, GAPC_COLOR_LOADPCT_KEY);
   pch_colors[2] = g_settings_get_string(pcfg->controller_settings, GAPC_COLOR_TIMELEFT_KEY);
   pch_colors[3] = g_settings_get_string(pcfg->controller_settings, GAPC_COLOR_BCHARGE_KEY);
   pch_colors[4] = g_settings_get_string(pcfg->controller_settings, GAPC_COLOR_BATTV_KEY);
   pch_colors[5] = NULL;
   ...
   if (plg != NULL) {
       for (i_series = 0; i_series < GAPC_LINEGRAPH_MAX_SERIES; i_series++) {
            ...
            g_clear_pointer (&pch_colors[i_series], g_free);
       }
 ...
 * return G_SOURCE_CONTINUE if successful, G_SOURCE_REMOVE otherwise
*/
static gboolean cb_util_line_chart_refresh(PGAPC_HISTORY pg)
{
   gint h_index = 0;
   ...
   g_return_val_if_fail(pg != NULL, G_SOURCE_REMOVE);

   pm = (PGAPC_MONITOR) pg->gp;
   plg = pg->plg;
   g_return_val_if_fail(plg != NULL, G_SOURCE_REMOVE);
   g_return_val_if_fail(pm != NULL, G_SOURCE_REMOVE);
...
static gint gapc_panel_graph_property_page(PGAPC_CONFIG pcfg, GtkWidget * notebook)
{
   ...
   * Prepare the top color choice area */
   ...
   gtk_box_pack_start(GTK_BOX(s_box), pbox, FALSE, FALSE, 0);
      ...
      cb_linev = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_LINEV_KEY);
   ...
   gtk_box_pack_start(GTK_BOX(s_box), pbox, FALSE, FALSE, 0);
      ...
      cb_loadpct = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_LOADPCT_KEY);
   ...
   gtk_box_pack_start(GTK_BOX(s_box), pbox, FALSE, FALSE, 0);
      ...
      cb_timeleft = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_TIMELEFT_KEY);
   ...
   gtk_box_pack_start(GTK_BOX(s_box), pbox, FALSE, FALSE, 0);
      ...
      cb_bcharge = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_BCHARGE_KEY);
   ...
   gtk_box_pack_start(GTK_BOX(s_box), pbox, FALSE, FALSE, 0);
      ...
      cb_battv = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_BATTV_KEY);
      ...
      g_hash_table_insert(pcfg->pht_Widgets, g_strdup(GAPC_COLOR_BATTV_KEY), cb_battv);
   ...
   gtk_box_pack_start(GTK_BOX(w_box), pbox, FALSE, FALSE, 0);
      ...
      cb_window = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_WINDOW_KEY);
   ...
   gtk_box_pack_start(GTK_BOX(w_box), pbox, FALSE, FALSE, 0);
      ...
      cb_chart = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_CHART_KEY);
   ...
   gtk_box_pack_start(GTK_BOX(w_box), pbox, FALSE, FALSE, 0);
      ...
      cb_text = gapc_color_button_new(pcfg->controller_settings, GAPC_COLOR_TITLE_KEY);
      ...
      g_hash_table_insert(pcfg->pht_Widgets, g_strdup(GAPC_COLOR_TITLE_KEY), cb_text);
